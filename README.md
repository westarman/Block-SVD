...
- memory efficiency of SVD matrices of an image need to be compared to original image memory (is this what the previous question pertains?) <-- probbly a  balance between **quality retention <-> memory compression**
- we need some ideas to increase quality/compression ratio (preprocessing with algorithms mybi? i have no clue tbh) <-- were gonna need to cook up something here...
